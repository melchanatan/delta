import numpy as np
import time
from my_visual_kinematics.Robot import *
...
class RobotTrajectory(object):
    ...
    def move_with_speed(self, speed, motion="p2p", method="linear", object_speed=0.2):
        ...
        for i in range(1, len(inter_values)):
            ...
            if not self.robot.is_reachable_inverse:
                print(f"Step {i}: Target position is out of reach. Waiting...")
                plt.pause(0.1)
                continue

            # Clear dynamic parts (collections and lines)
            # Clear lines and collections explicitly
